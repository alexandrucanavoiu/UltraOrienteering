...
<div class="modal fade" id="myModal-Participants-edit" data-keyboard="false" data-backdrop="static">
    <div class="modal-dialog">
        <div class="modal-content">
            ...
            <div class="col-lg-12 text-red">IMPORTANT: If you edit the participants and those participants have assigned Stage/Category/Route the data will be lost and you need to Enroll again the participants in a Stage, </div>
            <br /><br /><br />
            <form id="form">
                {{ csrf_field() }}
                            ...
                            <div class="div-left-input">
                                ...
                                <select id="clubs_id" name="clubs_id" class="form-control">
                                    ...
                                    @foreach($clubs as $club)
                                        <option value="{{ $club->id }}" @if($participant->clubs_id == $club->id) selected @endif>{{ $club->club_name }}</option>
                                    @endforeach
                                </select>
                            </div>
                        </div>
                ...
                <div class="modal-body">
                    @foreach($participant_manager as $participants)
                        <div class="added box box-primary bg-gray">
                            <div class="form-group box-body">
                                <label for="name">(<span>1</span>) Participant Name </label>
                                <div>
                                    <input class="form-control" name="participant_name[]" id="participant_name" type="text" value="{{ $participants->participant_name }}">
                                </div>
                                <br />
                                <div class="div-left-input">
                                    ...
                                    <select id="uuidcards_id" name="uuidcards_id[]" class="form-control participant_name">
                                        @foreach($uuidCards as $uuidcard)
                                            <option value="{{ $uuidcard->id }}" @if($participants->uuidcards_id == $uuidcard->id) selected @endif> No. #{{ $uuidcard->id }} - {{ $uuidcard->uuid_name }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
                ...
                <div class="modal-footer">
                    ...
                    <button type="submit" id="save" class="btn btn-primary js--ajax-form-edit-relay-participants-update" data-id="{{ $participant->id }}">Save</button>
                </div>
            </form>
        </div>
    </div>
